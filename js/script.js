...
{let a=parent.MyOOS_Dumper_content.location.href;let menuid=1;if(a.indexOf("config_overview.php")!=-1)menuid=2;if(a.indexOf("filemanagement.php")!=-1)
...
function setMenuActive(id){for(let i=1;i<=10;i++){let objid='m'+i;if(id==objid)
...
{let a="";let obj=document.getElementsByName("file[]");let anz=0;if(!obj.length)
...
return a;}
function Check(i,k)
{let anz=0;let s="";let smp;let ids=document.getElementsByName("file[]");let mp=document.getElementsByName("multipart[]");for(let j=0;j<ids.length;j++){if(ids[j].checked)
...
function Sel(v)
{let a=document.frm_tbl;if(!a.chk_tbl.length)
...
{let a=document.frm_tbl;a.tbl_array.value="";if(!a.chk_tbl.length)
...
function resizeSQL(i){let obj=document.getElementById("sqltextarea");let h=0;if(i==0){obj.style.height='4px';}else{if(i==1)h=-20;if(i==2)h=20;let oh=obj.style.height;let s=Number(oh.substring(0,oh.length-2))+h;if(s<24)s=24;obj.style.height=s+'px';}}
...
function InsertLib(i){let obj=document.getElementsByName('sqllib')[0];if(obj.selectedIndex>0){document.getElementById('sqlstring'+i).value=obj.options[obj.selectedIndex].value;document.getElementById('sqlname'+i).value=obj.options[obj.selectedIndex].text;}}
function DisplayExport(s){document.getElementById("export_working").InnerHTML=s;}
function SelectedTableCount(){let obj=document.getElementsByName('f_export_tables[]')[0];let anz=0;for(let i=0;i<obj.options.length;i++)
{if(obj.options[i].selected){anz++;}}
return anz;}
function SelectTableList(s){let obj=document.getElementsByName('f_export_tables[]')[0];for(let i=0;i<obj.options.length;i++){obj.options[i].selected=s;}}
