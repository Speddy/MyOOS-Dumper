...
{
    final public const PHP_7_3 = '7.3dev';
    final public const PHP_7_4 = '7.4dev';
    final public const PHP_8_0 = '8.0dev';
    final public const PHP_8_1 = '8.1dev';
    final public const PHP_8_2 = '8.2dev';
    ...
    private function fixupTokens()
    {
        ...
        $pos = 0;
        for ($i = 0, $c = is_countable($this->tokens) ? \count($this->tokens) : 0; $i < $c; $i++) {
            $token = $this->tokens[$i];
            if (\is_string($token)) {
                if ($patchPos === $pos) {
            ...
            $len = \strlen((string) $token[1]);
            $posDelta = 0;
            while ($patchPos >= $pos && $patchPos < $pos + $len) {
                $patchTextLen = \strlen((string) $patchText);
                if ($patchType === 'remove') {
                    if ($patchPos === $pos && $patchTextLen === $len) {
                        ...
                        $this->tokens[$i][1] = \substr_replace((string) $token[1], '', $patchPos - $pos + $posDelta, $patchTextLen);
                        $posDelta -= $patchTextLen;
                    }
                } elseif ($patchType === 'add') {
                    // Insert into the token string
                    $this->tokens[$i][1] = \substr_replace((string) $token[1], (string) $patchText, $patchPos - $pos + $posDelta, 0);
                    $posDelta += $patchTextLen;
                } else {
                    if ($patchType === 'replace') {
                        // Replace inside the token string
                        $this->tokens[$i][1] = \substr_replace((string) $token[1], (string) $patchText, $patchPos - $pos + $posDelta, $patchTextLen);
                    } else {
                        \assert(\false);
                    }
                }
    ...
    private function fixupErrors(array $errors)
    {
        foreach ($errors as $error) {
            ...
            foreach ($this->patches as $patch) {
                ...
                if ($patchType === 'add') {
                    $posDelta += \strlen((string) $patchText);
                    $lineDelta += \substr_count((string) $patchText, "\n");
                } else {
                    if ($patchType === 'remove') {
                        $posDelta -= \strlen((string) $patchText);
                        $lineDelta -= \substr_count((string) $patchText, "\n");
